...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_REQUEST['eid'])) {
		...
		$sql = "UPDATE tblenquiry SET Status=:status WHERE  isid=:eid";
		$query = $dbh->prepare($sql);
		$query->bindParam(':status', $status, PDO::PARAM_STR);
		$query->bindParam(':eid', $eid, PDO::PARAM_STR);
		$query->execute();

		$msg = "Đã duyệt thông tin";
	}
					...
					<div class="agile-tables">
						<div class="w3l-table-info">
							...
							<table id="table">
								...
								<tbody>
									<?php $sql = "SELECT tblissues.isid as usid,tblusers.FullName as fname,tblusers.MobileNumber as mnumber,tblusers.EmailId as email,tblissues.Issue as issue,tblissues.Description as Description,tblissues.PostingDate as PostingDate from tblissues join tblusers on tblusers.EmailId=tblissues.UserEmail";
									$query = $dbh->prepare($sql);
									$query->execute();
